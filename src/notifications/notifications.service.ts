...
export class NotificationsService {
  ...
  async getNotifications(userId: string, query: PaginateQueryDto): Promise<NotificationListResponse> {
    ...
    const [notifications, total, unreadCount] = await Promise.all([
      this.notificationModel.find({ userId: userObjectId })
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(limit)
        .exec(),
      this.notificationModel.countDocuments({ userId: userObjectId }).exec(),
      this.notificationModel.countDocuments({ userId: userObjectId, isRead: false }).exec(),
    ]);

    return {
      data: notifications.map((n) => new NotificationSerializer(n.toObject() as any)),
      unreadCount,
      meta: {
        total,
        page,
  ...
  async deleteNotification(notificationId: string, userId: string): Promise<{ deleted: boolean }> {
