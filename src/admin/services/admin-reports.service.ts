      ...
      if (report.type === ReportType.REVIEW) {
        try {
          ...
          if (review) {
            ...
            // NEW: notify the student whose review was removed
            await this.notificationModel.create({
              userId: review.studentId,
              title: 'Content Removed',
              message:
                'Your review was removed after being reported and reviewed by our team.',
          ...
          this.logger.error(
            `Failed to delete review ${report.targetId} during report resolution`,
            error,
          );
        }
      } else {
        this.logger.warn(
          `Action 'content_removed' requested for type '${report.type}' but not concretely implemented yet.`,
        );
      }
    }
