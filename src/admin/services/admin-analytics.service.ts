...
import { AnalyticsPeriodQueryDto, AnalyticsPeriod } from '../dto/analytics-period-query.dto';
...
export class AdminAnalyticsService {
  ...
  async getDashboardOverview(): Promise<AdminDashboardOverviewResponse> {
    ...
    const pendingApprovals = await this.courseModel.countDocuments({ courseStatus: CourseStatus.UNDER_REVIEW }).exec();
    const newSignupsToday = await this.userModel.countDocuments({ createdAt: { $gte: today } }).exec();
    const openReports = await this.reportModel.countDocuments({ status: ReportStatus.OPEN }).exec();
    
    const earningsAgg = await this.earningModel.aggregate([
  ...
  async getPlatformAnalytics(query: AnalyticsPeriodQueryDto): Promise<PlatformAnalyticsResponse> {
    ...
    if (period !== AnalyticsPeriod.ALL) {
      const days = period === AnalyticsPeriod.SEVEN_DAYS ? 7 : period === AnalyticsPeriod.THIRTY_DAYS ? 30 : 90;
      startDate = new Date();
    ...
    const totalInstructors = await this.userModel.countDocuments({ ...dateFilter, role: UserRole.INSTRUCTOR }).exec();
    const totalStudents = await this.userModel.countDocuments({ ...dateFilter, role: UserRole.STUDENT }).exec();
    const totalCourses = await this.courseModel.countDocuments({ ...dateFilter, courseStatus: CourseStatus.PUBLISHED }).exec();
    ...
    if (previousStartDate && startDate) {
      const prevEarningsAgg = await this.earningModel.aggregate([
        { $match: { createdAt: { $gte: previousStartDate, $lt: startDate } } },
        { $group: { _id: null, total: { $sum: '$amount' } } }
      ]).exec();
      const prevRevenue = prevEarningsAgg.length > 0 ? prevEarningsAgg[0].total : 0;
      if (prevRevenue > 0) {
    ...
    const topCourses = await this.courseModel.find({ courseStatus: CourseStatus.PUBLISHED })
      .sort({ totalEnrollments: -1 })
      .limit(5)
      .exec();

    const topInstructorsAgg = await this.earningModel.aggregate([
      ...(startDate ? [{ $match: { createdAt: { $gte: startDate } } }] : []),
    ...
    const topInstructorsPromises = topInstructorsAgg.map(async (agg) => {
      const instructorCourses = await this.courseModel.find({ instructorId: agg._id }).select('totalEnrollments').exec();
