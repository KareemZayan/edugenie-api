...
export class NotesService {
  ...
  async createNote(lessonId: string, dto: CreateNoteDto, studentId: string): Promise<NoteResponse> {
  ...
  async getNotesForLesson(lessonId: string, studentId: string): Promise<{ notes: NoteResponse[] }> {
    const hasAccess = await this.enrollmentsService.canAccessLesson(studentId, lessonId);
    if (!hasAccess) {
      throw new ForbiddenException('You must be enrolled in this course to view this lesson');
    }
    ...
    const notes = await this.noteModel
      .find({ studentId: new Types.ObjectId(studentId), lessonId: new Types.ObjectId(lessonId) })
      .sort({ timestamp: 1 })
